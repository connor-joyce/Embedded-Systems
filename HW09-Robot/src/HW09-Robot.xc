...
void controller(chanend motor_c, in port iEncoder, chanend dmp, chanend input, chanend output){
    ...
    message_t msg;
    timer t;
    unsigned int encoder_in, prev_encoder_in;
    ...
    while(1){
        ...
        select{
            ...
            case input :> msg:
                ...
                else if(!strncmp(msg.data, "init", 4)){
                    online = 1;
                    strcpy(msg.data, "INITIALIZING MOTOR COMMANDS");
                }
        ...
        //lock the motor behind an initializing command
        //prevents from reading output during wifi init and starting the motor
        //convience update
        if(online){
            motor_c <: msg.cmd;
        }
