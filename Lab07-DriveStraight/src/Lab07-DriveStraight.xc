...
void uart_to_console_task(chanend trigger_chan, chanend motor_chan, chanend encoder_chan, chanend encoder_change,
    ...
    while(1){
        ...
        //if car is in straight drive mode then check to see if the car is veering off course
        else if(straight_drive){
            int l = cmd.left_duty_cycle;
            int r = cmd.right_duty_cycle;
            float delta = 10 * abs(starting_yaw - yaw);
            //this more or less works
            //hitting fringe cases where there is a very big error and the car loses control
            //currently working, will attempt to fix errors, I think the error is caused by an
            //outlier yaw reading during driving, the car spins wildly to attempt to course correct.
            //stopping it and restarting it (resetting target yaw) fixes it so this is the only explanation I have

            //if veering to the right
            if(yaw >= (starting_yaw + DRIVE_ERROR)){
                if(abs(cmd.right_duty_cycle) >= 35){
                    //add the delta in the proper sign case (+/-)
                    if(r > 0){
                        cmd.right_duty_cycle += delta;
                    }
                    else{
                        cmd.right_duty_cycle -= delta;
                    }
                }
                //or remove the delta in the proper sign case (+/-)
                else if(abs(cmd.left_duty_cycle) <= 75){
                    if(l > 0){
                        cmd.left_duty_cycle -= delta;
                    }
                    else{
                        cmd.left_duty_cycle += delta;
                    }
            ...
            //do the same on the other side if veering to the left
            else if(yaw <= (starting_yaw - DRIVE_ERROR)){
                if(abs(cmd.left_duty_cycle) >= 35){
                    if(l > 0){
                        cmd.left_duty_cycle += delta;
                    }
                    else{
                ...
                else if(abs(cmd.right_duty_cycle) <= 75){
                    if(r > 0){
                        cmd.right_duty_cycle -= delta;
                    }
                    else{
